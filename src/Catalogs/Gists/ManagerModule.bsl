...
Процедура ПрочитатьGists() Экспорт
	...
	Если ПустаяСтрока(GistsПовтИсп.ТокенПользователя()) Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   |			И (Gists.Пользователь = &Пользователь)
				   |ГДЕ
	...
	Пока ВыборкаGists.Следующий() Цикл
		...
		НовыйGist.Пользователь = Пользователь;
		...
		Пока Выборка.Следующий() Цикл
			Если Выборка.Ссылка = Null Тогда
				...
				НовыйТег.Пользователь = Пользователь;
...
Функция ВызватьHTTPМетод(Идентификатор = "", HTTPМетод = "GET", Данные = Неопределено, КодСостояния = 200)
	...
	ЗапросHTTP.Заголовки.Вставить("Authorization", "token " + GistsПовтИсп.ТокенПользователя());
